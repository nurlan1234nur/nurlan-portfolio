...
export function SkillGroup({
  ...
  return (
    <div>
      <p className="mb-6 font-mono text-[9px] tracking-widest text-white/30">{label}</p>
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span
            key={skill}
            className={`border px-3 py-2 font-mono text-[10px] ${accent ? "border-accent/25 bg-accent/[.04] text-accent/75" : "border-white/10 text-white/35"}`}
          >
            {skill}
          </span>
        ))}
...
export function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="min-w-28">
      ...
      <p className="mt-1 font-mono text-[8px] tracking-widest text-white/30">{label}</p>
    </div>
  )
}
...
export function Capability({
  ...
  return (
    <article className="border-white/10 py-8 md:border-r md:px-7 md:first:pl-0 md:last:border-r-0">
      ...
      <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/40">{body}</p>
    </article>
  )
}
  ...
  return (
    <div className="relative pb-12 pl-8 before:absolute before:-left-1 before:top-1 before:h-2 before:w-2 before:rounded-full before:border before:border-accent before:bg-[#0b0c0c]">
      <div className="grid gap-3 md:grid-cols-[150px_1fr]">
        <span className="font-mono text-[9px] tracking-widest text-accent/60">{year}</span>
        <div>
...
export function Contact({ label, value, href }: { label: string; value: string; href?: string }) {
  const content: ReactNode = (
    <>
      <span className="font-mono text-[9px] tracking-widest text-white/25">{label}</span>
